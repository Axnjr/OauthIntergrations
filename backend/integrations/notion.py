...
CLIENT_ID = safequote(os.environ.get('NOTION_OAUTH_CLIENT_ID', '000'))
CLIENT_SECRET = safequote(os.environ.get('NOTION_OAUTH_CLIENT_SECRET', '000'))

# SECURITY
if(CLIENT_ID == "000" or CLIENT_SECRET == "000"):
    raise Exception("\n Notion 'CLIENT_ID' & 'CLIENT_SECRET' Not Found !!")
...
async def authorize_notion(user_id, org_id):
    ...
    # -- DEBUG -- #
    print("Added org_id and user_id to redis !!", CLIENT_ID, CLIENT_SECRET)

    ans = f'{authorization_url}&state={encoded_state}'
...
async def oauth2callback_notion(request: Request):
    ...
    print("\n DATA FROM NOTION PROVIDER STORED IN REDIS SERVER !! \n")
